"""Lightweight HTTP client for the Proof Protocol node — no LangChain dependency."""
...
class CPOClient:
    ...
    def node_info(self) -> dict:
        """Return node identity, worlds, and ledger stats."""
        resp = requests.get(f"{self.base_url}/node", timeout=self.timeout)
        resp.raise_for_status()
        return resp.json()
