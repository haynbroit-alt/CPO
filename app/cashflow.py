...
class CPORecord(BaseModel):
    """Public proof index — one record per completed audit."""
    cpo: str
    session_id: str
    audit_date: str
    findings_count: int
...
_cpo_index: SqliteStore[CPORecord] = SqliteStore("cpo_index", CPORecord)
...
def _execute_audit(session_id: str) -> None:
    ...
    # Index CPO for public verification
    _cpo_index.set(cpo, CPORecord(
        cpo=cpo,
        session_id=session_id,
        audit_date=_now(),
        ...
        is_demo=sess.filename == "demo_transactions.csv",
    ))
...
@app.get("/verify/{cpo}", response_class=HTMLResponse)
def verify_page(cpo: str) -> HTMLResponse:
    """Human-readable proof verification page."""
    page = _STATIC_DIR / "verify.html"
    if page.exists():
        return HTMLResponse(page.read_text().replace("{{CPO}}", cpo))
    return HTMLResponse(_fallback_verify(cpo))


@app.get("/api/verify/{cpo}")
def verify_api(cpo: str) -> dict:
    """Machine-readable proof verification — returns JSON."""
    record = _cpo_index.get(cpo)
    if not record:
        raise HTTPException(404, "CPO not found — not generated by this server")
    return {
        "valid": True,
        "cpo": record.cpo,
        "audit_date": record.audit_date,
        "findings_count": record.findings_count,
        "total_detected": record.total_detected,
        "currency": record.currency,
        "transactions_analyzed": record.transactions_analyzed,
        "reproducible": True,
        "is_demo": record.is_demo,
        "verified_by": _BASE_URL,
    }
...
@app.get("/stats")
def stats() -> dict:
    """Aggregate public stats — no PII, anonymous."""
    records = _cpo_index.values()
    real = [r for r in records if not r.is_demo]
    return {
        "total_audits": len(real),
        "total_detected_eur": round(sum(r.total_detected for r in real), 2),
        "total_findings": sum(r.findings_count for r in real),
    }
...
def demo() -> RedirectResponse:
    """Free demo — run sample CSV without payment, redirect to results."""
    session_id = str(uuid.uuid4())
    _sessions.set(session_id, AuditSession(
        id=session_id,
        csv_b64=base64.b64encode(_DEMO_CSV).decode(),
    ...
    return RedirectResponse(f"/results/{session_id}", status_code=302)
...
def _fallback_verify(cpo: str) -> str:
    return (
        f"<!DOCTYPE html><html><head><title>SIOS Proof</title></head><body>"
        f"<script>fetch('/api/verify/{cpo}')"
        f".then(r=>r.json()).then(d=>document.body.innerHTML='<pre>'+JSON.stringify(d,null,2)+'</pre>')"
        f".catch(()=>document.body.innerHTML='CPO not found');"
        f"</script></body></html>"
    )
